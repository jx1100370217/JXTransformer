...
def load_en_vocab():
    vocab = [line.split()[0] for line in codecs.open('vocab/vocab.en','r','utf-8').read().splitlines()
             if int(line.split()[1])>=hp.min_cnt]
...
def create_data(source_sents,target_sents):
    ...
    for source_sent,target_sent in zip(source_sents,target_sents):
        ...
        y = [zh2idx.get(word,1) for word in (target_sent+u" </S>").split()]
...
def load_train_data():
    ...
    en_sents = [_refine(line) for line in codecs.open(hp.source_train, 'r', 'utf-8').read().split('\n') if
                line and line[0] != "<"]
    zh_sents = [_refine(line) for line in codecs.open(hp.target_train, 'r', 'utf-8').read().split('\n') if
                line and line[0] != '<']
...
def load_test_data():
    ...
    zh_sents = [_refine(line) for line in codecs.open(hp.target_test,'r','utf-8').read().split('\n') if line and line[:4] == '<seg']
